...
uint32_t gPaintValue = 1;               // 1 = make alive, 0 = kill
int gBrushRadius = 0;                   // 0 = single cell, >0 = disc radius in cells
int gLastCellX = -1, gLastCellY = -1;   // avoid repainting same cell
...
static constexpr float UPDATE_INTERVAL_MS = 200.0f; // 5 Hz, matches JS
...
wgpu::ShaderModule renderModule;
...
wgpu::Buffer cellStateStorage[2];
...
void ConfigureSurface() {
  ...
  wgpu::SurfaceConfiguration config{};   // zero-initialise
  config.device = device;
  config.format = format;
  config.usage = wgpu::TextureUsage::RenderAttachment;
#if defined(__EMSCRIPTEN__)
  config.alphaMode = wgpu::CompositeAlphaMode::Premultiplied; // typical for canvas
#else
...
void CreateRenderPipelineAndVertexBuffer() {
  renderModule = CreateWGSL(kCellRenderWGSL);
  ...
  vertexCount = static_cast<uint32_t>(verts.size() / 2);
  ...
  device.GetQueue().WriteBuffer(vertexBuffer, 0, verts.data(), vbDesc.size);
  ...
  wgpu::ColorTargetState colorTarget{ .format = format };
  ...
  rpDesc.vertex.module = renderModule;
  ...
  renderPipeline = device.CreateRenderPipeline(&rpDesc);
}

void CreateComputePipeline() {
  computeModule = CreateWGSL(kLifeComputeWGSL);

  wgpu::ComputePipelineDescriptor cpDesc{
      .layout = pipelineLayout,
      .compute = { .module = computeModule, .entryPoint = "computeMain" }
  };
  computePipeline = device.CreateComputePipeline(&cpDesc);
}
...
void CreateBuffersAndBindGroups() {
  ...
  device.GetQueue().WriteBuffer(uniformBuffer, 0, gridData.data(), sizeof(gridData));
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    cellStateStorage[i] = device.CreateBuffer(&sbDesc);
  }
  ...
  // Initialise buffer 0 with random states like the JS
  {
    ...
    device.GetQueue().WriteBuffer(cellStateStorage[0], 0, cells.data(), cellBytes);
  }
  // Initialise buffer 1 with a striped pattern like the JS second grid
  {
    ...
    device.GetQueue().WriteBuffer(cellStateStorage[1], 0, cells.data(), cellBytes);
  }
  ...
  // A: in = 0, out = 1
  {
    ...
    entries[0] = wgpu::BindGroupEntry{ .binding = 0, .buffer = uniformBuffer, .offset = 0, .size = sizeof(float)*2 };
    entries[1] = wgpu::BindGroupEntry{ .binding = 1, .buffer = cellStateStorage[0], .offset = 0, .size = cellBytes };
    entries[2] = wgpu::BindGroupEntry{ .binding = 2, .buffer = cellStateStorage[1], .offset = 0, .size = cellBytes };
    ...
    bindGroups[0] = device.CreateBindGroup(&bgDesc);
  }
  // B: in = 1, out = 0
  {
    std::array<wgpu::BindGroupEntry,3> entries{};
    entries[0] = wgpu::BindGroupEntry{ .binding = 0, .buffer = uniformBuffer, .offset = 0, .size = sizeof(float)*2 };
    entries[1] = wgpu::BindGroupEntry{ .binding = 1, .buffer = cellStateStorage[1], .offset = 0, .size = cellBytes };
    entries[2] = wgpu::BindGroupEntry{ .binding = 2, .buffer = cellStateStorage[0], .offset = 0, .size = cellBytes };
    ...
    bindGroups[1] = device.CreateBindGroup(&bgDesc);
  }
}
...
static bool ScreenToCell(double sx, double sy, uint32_t& cx, uint32_t& cy) {
  if (sx < 0 || sy < 0 || sx >= kWidth || sy >= kHeight) return false;

  cx = static_cast<uint32_t>(sx / double(kWidth)  * GRID_SIZE);

  // GLFW reports y from top. Our grid.y increases upward in the shader, so invert.
  uint32_t yTop = static_cast<uint32_t>(sy / double(kHeight) * GRID_SIZE);
  cy = GRID_SIZE - 1 - yTop;
...
static void PaintCell(uint32_t cx, uint32_t cy, uint32_t value) {
  ...
  for (int b = 0; b < 2; ++b) {
    device.GetQueue().WriteBuffer(cellStateStorage[b], byteOffset, &value, sizeof(value));
  }
}

static void PaintAtCursor() {
  uint32_t cx, cy;
  if (!ScreenToCell(gMouseX, gMouseY, cx, cy)) return;

  if (gBrushRadius == 0) {
    if (int(cx) == gLastCellX && int(cy) == gLastCellY) return; // no duplicate writes
    PaintCell(cx, cy, gPaintValue);
  } else {
    // small disc brush
    for (int dy = -gBrushRadius; dy <= gBrushRadius; ++dy)
      for (int dx = -gBrushRadius; dx <= gBrushRadius; ++dx) {
        if (dx*dx + dy*dy > gBrushRadius*gBrushRadius) continue;
        int nx = int(cx) + dx, ny = int(cy) + dy;
        if (nx < 0 || ny < 0 || nx >= int(GRID_SIZE) || ny >= int(GRID_SIZE)) continue;
        PaintCell(uint32_t(nx), uint32_t(ny), gPaintValue);
      }
  }
  gLastCellX = int(cx); gLastCellY = int(cy);
}

static void CursorPosCallback(GLFWwindow* w, double x, double y) {
  gMouseX = x; gMouseY = y;
  if (gMouseDown) PaintAtCursor(); // smooth painting while dragging
}
...
static void MouseButtonCallback(GLFWwindow* w, int button, int action, int mods) {
  if (button == GLFW_MOUSE_BUTTON_LEFT || button == GLFW_MOUSE_BUTTON_RIGHT) {
    if (action == GLFW_PRESS) {
      ...
      gPaintValue = (button == GLFW_MOUSE_BUTTON_RIGHT || (mods & GLFW_MOD_SHIFT)) ? 0u : 1u;
      gLastCellX = gLastCellY = -1;
...
void Frame() {
    ...
    const double dtMs = std::chrono::duration<double, std::milli>(now - lastTick).count();
    ...
    if (doUpdateNow)
        accumulatorMs -= UPDATE_INTERVAL_MS;   // only drain when we will compute
  #endif
    ...
    wgpu::SurfaceTexture surfaceTexture;
      ...
      if (doUpdateNow) {
        ...
        cpass.SetBindGroup(0, bindGroups[stepIndex % 2]);  // in=step%2, out=other
      ...
      // 2) Render every frame, sampling the output buffer of the current step.
      {
        ...
        pass.SetBindGroup(0, bindGroups[(stepIndex + 1) % 2]); // read freshly written side
        pass.SetVertexBuffer(0, vertexBuffer, 0, WGPU_WHOLE_SIZE);
        pass.Draw(vertexCount, GRID_SIZE * GRID_SIZE, 0, 0);
        pass.End();
      }
...
void Start() {
  ...
  GLFWwindow* window =
      glfwCreateWindow(kWidth, kHeight, "WebGPU Life (Dawn + GLFW)", nullptr, nullptr);
